...
class JWTGeneration(BaseModel):
    """
    Serializer for JWT token generation for API users
    """ # noqa: E501
    email: StrictStr = Field(description="Email address of the API user")
    password: StrictStr = Field(description="Password for the API user")
    organisation_id: Optional[UUID] = Field(default=None, description="Organization ID to generate token for")
    division_id: Optional[UUID] = Field(default=None, description="Division ID to generate token for (optional)")
    __properties: ClassVar[List[str]] = ["email", "password", "organisation_id", "division_id"]
