/**
 * Pescheck API
 * Official client library for the Pescheck API (v2), generated from the OpenAPI specification. Authenticate with OAuth2 client credentials and use the checks, profiles, screenings, webhooks and divisions endpoints. See https://github.com/pescheckit/pescheck-clients for installation and usage.
 *
 * The version of the OpenAPI document: 2.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The V2ScreeningNoteInput model module.
 * @module model/V2ScreeningNoteInput
 * @version 0.1.0
 */
class V2ScreeningNoteInput {
    /**
     * Constructs a new <code>V2ScreeningNoteInput</code>.
     * A note to record on the screening at creation. Scope it to a specific check with &#x60;profile_check_id&#x60; (links the note to the Check spawned from that ProfileCheck) or to a &#x60;check_type&#x60;; &#x60;profile_check_id&#x60; wins when both are given. With neither, the note is recorded against the screening with no check (check_type &#39;other&#39;).
     * @alias module:model/V2ScreeningNoteInput
     * @param note {String} 
     */
    constructor(note) { 
        
        V2ScreeningNoteInput.initialize(this, note);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, note) { 
        obj['note'] = note;
    }

    /**
     * Constructs a <code>V2ScreeningNoteInput</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/V2ScreeningNoteInput} obj Optional instance to populate.
     * @return {module:model/V2ScreeningNoteInput} The populated <code>V2ScreeningNoteInput</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new V2ScreeningNoteInput();

            if (data.hasOwnProperty('note')) {
                obj['note'] = ApiClient.convertToType(data['note'], 'String');
            }
            if (data.hasOwnProperty('profile_check_id')) {
                obj['profile_check_id'] = ApiClient.convertToType(data['profile_check_id'], 'String');
            }
            if (data.hasOwnProperty('check_type')) {
                obj['check_type'] = ApiClient.convertToType(data['check_type'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>V2ScreeningNoteInput</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>V2ScreeningNoteInput</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of V2ScreeningNoteInput.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // ensure the json data is a string
        if (data['note'] && !(typeof data['note'] === 'string' || data['note'] instanceof String)) {
            throw new Error("Expected the field `note` to be a primitive type in the JSON string but got " + data['note']);
        }
        // ensure the json data is a string
        if (data['profile_check_id'] && !(typeof data['profile_check_id'] === 'string' || data['profile_check_id'] instanceof String)) {
            throw new Error("Expected the field `profile_check_id` to be a primitive type in the JSON string but got " + data['profile_check_id']);
        }
        // ensure the json data is a string
        if (data['check_type'] && !(typeof data['check_type'] === 'string' || data['check_type'] instanceof String)) {
            throw new Error("Expected the field `check_type` to be a primitive type in the JSON string but got " + data['check_type']);
        }

        return true;
    }


}

V2ScreeningNoteInput.RequiredProperties = ["note"];

/**
 * @member {String} note
 */
V2ScreeningNoteInput.prototype['note'] = undefined;

/**
 * Links the note to the Check spawned from this ProfileCheck. Wins over check_type.
 * @member {String} profile_check_id
 */
V2ScreeningNoteInput.prototype['profile_check_id'] = undefined;

/**
 * Scope the note to a check type. Ignored when profile_check_id is given.  * `addresscheck` - addresscheck * `adversemedia2check` - adversemedia2check * `adversemediacheck` - adversemediacheck * `bigcheck` - bigcheck * `criminalrecordscheck` - criminalrecordscheck * `criminalrecordsuploadcheck` - criminalrecordsuploadcheck * `customintegritycheck` - customintegritycheck * `cvcheck` - cvcheck * `edrcheck` - edrcheck * `focumcheck` - focumcheck * `id2check` - id2check * `idcheck` - idcheck * `integritycheck` - integritycheck * `openhealthcarecheck` - openhealthcarecheck * `permissioncheck` - permissioncheck * `pescheckadversemediacheck` - pescheckadversemediacheck * `qualificationcheck` - qualificationcheck * `righttoworkcheck` - righttoworkcheck * `vogcheck` - vogcheck * `watchlist2check` - watchlist2check * `watchlistcheck` - watchlistcheck * `workreferencecheck` - workreferencecheck * `worldwidecreditcheck` - worldwidecreditcheck
 * @member {module:model/V2ScreeningNoteInput.CheckTypeEnum} check_type
 */
V2ScreeningNoteInput.prototype['check_type'] = undefined;





/**
 * Allowed values for the <code>check_type</code> property.
 * @enum {String}
 * @readonly
 */
V2ScreeningNoteInput['CheckTypeEnum'] = {

    /**
     * value: "addresscheck"
     * @const
     */
    "addresscheck": "addresscheck",

    /**
     * value: "adversemedia2check"
     * @const
     */
    "adversemedia2check": "adversemedia2check",

    /**
     * value: "adversemediacheck"
     * @const
     */
    "adversemediacheck": "adversemediacheck",

    /**
     * value: "bigcheck"
     * @const
     */
    "bigcheck": "bigcheck",

    /**
     * value: "criminalrecordscheck"
     * @const
     */
    "criminalrecordscheck": "criminalrecordscheck",

    /**
     * value: "criminalrecordsuploadcheck"
     * @const
     */
    "criminalrecordsuploadcheck": "criminalrecordsuploadcheck",

    /**
     * value: "customintegritycheck"
     * @const
     */
    "customintegritycheck": "customintegritycheck",

    /**
     * value: "cvcheck"
     * @const
     */
    "cvcheck": "cvcheck",

    /**
     * value: "edrcheck"
     * @const
     */
    "edrcheck": "edrcheck",

    /**
     * value: "focumcheck"
     * @const
     */
    "focumcheck": "focumcheck",

    /**
     * value: "id2check"
     * @const
     */
    "id2check": "id2check",

    /**
     * value: "idcheck"
     * @const
     */
    "idcheck": "idcheck",

    /**
     * value: "integritycheck"
     * @const
     */
    "integritycheck": "integritycheck",

    /**
     * value: "openhealthcarecheck"
     * @const
     */
    "openhealthcarecheck": "openhealthcarecheck",

    /**
     * value: "permissioncheck"
     * @const
     */
    "permissioncheck": "permissioncheck",

    /**
     * value: "pescheckadversemediacheck"
     * @const
     */
    "pescheckadversemediacheck": "pescheckadversemediacheck",

    /**
     * value: "qualificationcheck"
     * @const
     */
    "qualificationcheck": "qualificationcheck",

    /**
     * value: "righttoworkcheck"
     * @const
     */
    "righttoworkcheck": "righttoworkcheck",

    /**
     * value: "vogcheck"
     * @const
     */
    "vogcheck": "vogcheck",

    /**
     * value: "watchlist2check"
     * @const
     */
    "watchlist2check": "watchlist2check",

    /**
     * value: "watchlistcheck"
     * @const
     */
    "watchlistcheck": "watchlistcheck",

    /**
     * value: "workreferencecheck"
     * @const
     */
    "workreferencecheck": "workreferencecheck",

    /**
     * value: "worldwidecreditcheck"
     * @const
     */
    "worldwidecreditcheck": "worldwidecreditcheck",

    /**
     * value: ""
     * @const
     */
    "empty": ""
};



export default V2ScreeningNoteInput;

